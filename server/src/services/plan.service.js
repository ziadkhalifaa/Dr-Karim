...
export const planService = {
  ...
  async activate(domain, { versionId, tenantId, auth }) {
    ...
    return sequelize.transaction(async (transaction) => {
      ...
      try {
        const code = domain === "nutrition" ? "nutrition_plan" : "exercise_plan";
        await entitlementService.requireEntitlement({ tenantId, patientId: plan.patient_id, code, transaction });
      } catch (err) {
        if (err.code !== "ENTITLEMENT_REQUIRED" && err.code !== "SUBSCRIPTION_REQUIRED" && err.code !== "SUBSCRIPTION_EXPIRED") throw err;
        // Bypass entitlement check for doctor manual plan activation
      }
